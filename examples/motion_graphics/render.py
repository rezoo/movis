...
def circle_anim(
        ...
        duration: float, sizes: tuple[int, int], motion_type: str):
    layer = mv.layer.Ellipse(
        (sizes[0], sizes[0]),
        contents=[mv.layer.StrokeProperty(mv.to_rgb(color), 5.0)],
        duration=duration)
    item = scene.add_layer(layer, offset=offset)
    layer.size.enable_motion().extend([0, duration], [sizes[0], sizes[1]], [motion_type])
    item.transform.opacity.enable_motion().extend([0, duration], [1, 0], [motion_type])
...
def main():
    ...
    scene.add_layer(mv.layer.Rectangle(size, color=bg_color, duration=duration))
    scene.add_layer(
    ...
    circle_anim(scene, color=circle_color, offset=0.0, duration=1.0, sizes=(0, 1200), motion_type='ease_out5')
    circle_anim(scene, color=circle_color, offset=0.2, duration=1.0, sizes=(0, 2200), motion_type='ease_out5')
    circle_anim(scene, color=circle_color, offset=1.6, duration=1.0, sizes=(2200, 0), motion_type='ease_out5')
    ...
    circle_anim(scene, color=bg_color, offset=2.25, duration=1.0, sizes=(0, 2000), motion_type='ease_out5')
    ...
    scene.add_layer(
        mv.layer.Text(
            ...
            font_style='Light', color="#ffffff", duration=duration),
        transform=mv.Transform(position=(p[0], p[1] - 50)),
